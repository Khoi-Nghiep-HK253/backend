...
5. Nếu categoryId != null → tìm Category → 404 nếu không có
6. Load tất cả GroupMember → tạo userMap (userId → User)

── Validate dữ liệu đầu vào:
7. validatePayers: tổng tiền payers phải == totalAmount, userId phải là thành viên group
8. validateShares: userId phải là thành viên group, ràng buộc riêng theo SplitType

── Tạo & lưu Expense:
9. Tạo Expense entity (MapStruct: group, currency, category, splitType, ...)
10. Lưu Expense vào DB

── Tạo & lưu ExpensePayer:
11. Với mỗi payer trong request → tạo ExpensePayer { expense, user, amount }
12. Lưu tất cả vào DB

── Tính toán phần chia (normalizedSharesMap):
13. Theo SplitType → calculateNormalizedShares() → Map<userId, amount>
14. Lưu ExpenseShare cho từng user

── Tính toán & lưu Debt:
15. calculateAndSaveDebts():
...
16. Lưu tất cả Debt vào DB

17. Trả về ExpenseResponse (expense + payers + shares + debts)
```
...
```
GET /api/groups/{groupId}/expenses?categoryId=&fromDate=&toDate=&page=&size=
Authorization: Bearer <token>
```
...
7. Validate currency, category, payers, shares (giống khi tạo)
