...
`Category` (`entity/Category.java`) tags a **Group** (e.g. "Du lịch", "Ăn uống") — `Expense` has no category field, only `Currency`. `POST`/`PUT /api/groups` pick a category one of two mutually-exclusive ways (`GroupValidator.validateCategorySelection` rejects sending both as 400): `categoryId` looks up an existing category and 404s if it doesn't exist (same strict pattern as everywhere else), while `categoryName` finds one case-insensitively or **creates it on the spot if none matches** (`GroupServiceImpl.resolveCategory`) — this is the one path in the codebase that never 404s on a missing category. The standalone `POST /api/categories` (`CategoryServiceImpl`/`CategoryValidator`) still validates strictly (name uniqueness, 404 on unknown id) for direct category CRUD.
...
- Secrets/env vars are loaded via dotenv (`.env`, see `.env.example`): `JWT_SECRET`, Resend email vars (`RESEND_API_URL`, `RESEND_API_KEY`, `APP_MAIL_FROM`, `APP_MAIL_ENABLED`), Cloudinary vars (`CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`), `GEMINI_API_KEY`/`GEMINI_MODEL` (AI receipt scanning), and `APP_BASE_URL` (used in emailed links, e.g. password reset).
...
`AiConfig` (`config/AiConfig.java`) exposes a `ChatClient` bean built from the Spring AI Google GenAI auto-configuration (`spring.ai.google.genai.*` in `application.yml`; uses the Gemini Developer API key mode, not Vertex AI). Both current AI features are **preview, non-persisted** calls: the client reviews/edits the suggestion and then calls the real create endpoint itself.

- `ReceiptScanService`/`ReceiptScanServiceImpl` — `POST /api/groups/{groupId}/expenses/scan-receipt` sends a receipt photo to Gemini (multimodal) and returns a `ReceiptScanResponse` (description, totalAmount, currency, lineItems) to pre-fill the client's expense form — payers, shares, split type and date are deliberately left to the user, who then calls the real `POST /expenses`. AI output is parsed into `ReceiptExtraction` (`service/model/ReceiptExtraction.java`).
- `GroupCategorySuggestionService`/`GroupCategorySuggestionServiceImpl` — `POST /api/groups/suggest-category` sends the group's name/note plus the list of existing category names to Gemini (text-only, no media) and gets back one category name, either an existing one or a new proposal. The service then resolves that name against `CategoryRepository` itself (case-insensitive) rather than trusting an AI-picked ID, and returns `SuggestCategoryResponse` shaped exactly like `CreateGroupRequest`'s `categoryId`/`categoryName` fields so the client can drop it straight into the create-group form. AI output is parsed into `GroupCategorySuggestion` (`service/model/GroupCategorySuggestion.java`).

In both cases the AI's raw JSON output is parsed into an internal record via `ChatClient`'s structured-output `.entity(Class)` — never expose those records directly through the API, always map into a public response DTO. The provider-facing code only uses Spring AI's provider-agnostic `ChatClient`/`Media` APIs, never Gemini-specific types, so swapping providers again is a dependency + config change only. Follow this pattern (system+user prompt, `.entity(...)`, resolve/validate against real data yourself, map into a public response DTO) for future AI features.
