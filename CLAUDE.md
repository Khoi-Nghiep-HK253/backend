...
`AiConfig` (`config/AiConfig.java`) exposes a `ChatClient` bean built from the Spring AI Google GenAI auto-configuration (`spring.ai.google.genai.*` in `application.yml`; uses the Gemini Developer API key mode, not Vertex AI). Current usage: `ReceiptScanService`/`ReceiptScanServiceImpl` — `POST /api/groups/{groupId}/expenses/scan-receipt` sends a receipt photo to Gemini (multimodal) and returns a **non-persisted** `ReceiptScanResponse` (description, totalAmount, currency, lineItems) used to pre-fill the client's expense form — payers, shares, split type and date are deliberately left to the user, who then calls the real `POST /expenses`. The AI's raw JSON output is parsed into the internal `ReceiptExtraction` record (`service/model/ReceiptExtraction.java`) via `ChatClient`'s structured-output `.entity(Class)` — never expose that record directly through the API. The provider-facing code (`AiConfig`, `ReceiptScanServiceImpl`) only uses Spring AI's provider-agnostic `ChatClient`/`Media` APIs, never Gemini-specific types, so swapping providers again is a dependency + config change only. Follow this pattern (system+user prompt, `.entity(...)`, map into a public response DTO) for future AI features.
